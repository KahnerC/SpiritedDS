...
std::string name[32];
std::string card_number[32];
int v_mental[32];//Mental state, 0 to 10 "SOUND MIND"
int v_spooky[32];//Ghostly powers, 0 to 10 "UNCANNY TALENT"
int v_evil[32];//Wickedness, 0 to 10 "WICKED DEEDS"
int v_life[32];//Expected lifespan, -10 to 10 "LASTING HEALTH"
int v_impact[32];//Impact on the world, 0 to 10 "OF CONSEQUENCE"
...
std::string c1img = "sprite/" + card_number[card1] + "x";
std::string c2img = "sprite/" + card_number[card2] + "x";
...
while (press_start == false) {
	if (keys & KEY_START) {press_start = true;}
	rand_roll = rand();
	scanKeys();
	keys = keysDown();
}
...
card1 = deck1[0];
card2 = deck2[0];
c1img = "sprite/" + card_number[card1] + "x";
c2img = "sprite/" + card_number[card2] + "x";
//std::string tltest = "sprite/007";
//char tlimg[10];
//strcpy(tlimg, tltest);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c1img + "1").c_str(), 0, 64, 64);
    NF_LoadSpritePal((c1img + "1").c_str(), 0);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(1, 0, 0, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(1, 0, 0);
NF_CreateSprite(1, 0, 0, 0, 8, 28);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c1img + "2").c_str(), 1, 64, 64);
    NF_LoadSpritePal((c1img + "2").c_str(), 1);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(1, 1, 1, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(1, 1, 1);
NF_CreateSprite(1, 1, 1, 1, 72, 28);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c1img + "3").c_str(), 2, 64, 64);
    NF_LoadSpritePal((c1img + "3").c_str(), 2);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(1, 2, 2, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(1, 2, 2);
NF_CreateSprite(1, 2, 2, 2, 8, 92);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c1img + "4").c_str(), 3, 64, 64);
    NF_LoadSpritePal((c1img + "4").c_str(), 3);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(1, 3, 3, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(1, 3, 3);
NF_CreateSprite(1, 3, 3, 3, 72, 92);


    // Load sprite files from NitroFS - Other screen
    NF_LoadSpriteGfx((c2img + "1").c_str(), 4, 64, 64);
    NF_LoadSpritePal((c2img + "1").c_str(), 4);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(0, 4, 4, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(0, 4, 4);
NF_CreateSprite(0, 4, 4, 4, 8, 28);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c2img + "2").c_str(), 5, 64, 64);
    NF_LoadSpritePal((c2img + "2").c_str(), 5);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(0, 5, 5, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(0, 5, 5);
NF_CreateSprite(0, 5, 5, 5, 72, 28);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c2img + "3").c_str(), 6, 64, 64);
    NF_LoadSpritePal((c2img + "3").c_str(), 6);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(0, 6, 6, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(0, 6, 6);
NF_CreateSprite(0, 6, 6, 6, 8, 92);

    // Load sprite files from NitroFS
    NF_LoadSpriteGfx((c2img + "4").c_str(), 7, 64, 64);
    NF_LoadSpritePal((c2img + "4").c_str(), 7);
    // Transfer the required sprites to VRAM
    NF_VramSpriteGfx(0, 7, 7, true); // Ball: Keep all frames in VRAM
    NF_VramSpritePal(0, 7, 7);
NF_CreateSprite(0, 7, 7, 7, 72, 92);
...
//strcpy(tlimg, tltest);
	...
	NF_WriteText(0, 0, 2, 20, str_buffer);
...
//Round processing end
	...
	NF_WriteText(1, 1, 2, 20, str_buffer);
