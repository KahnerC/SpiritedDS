...
diff[6] = 5;diff_disp[5] = "AI Cheats";
...
while (not start_game) {
	...
	if ((keys & KEY_A)||(keys & KEY_B)) {
		...
		if (menu_select == 1) {jank = (jank_select + 1) % jank_total;}//IMAGE COMPRESSION
		if (menu_select == 2) {diff = (diff_select + 1) % diff_total;}//DIFFICULTY
		if (menu_select == 3) {//ABOUT
			keep_text = true;
			NF_ClearTextLayer(1, 1);
			NF_ClearTextLayer(0, 0);
			NF_WriteText(1, 1, 0, 0, "Spirited is a 2010 TV series about a rock star ghost who falls in love with Australian dentist. I am not making it up. Top Trumps is a long-dated ghost of a card game with thousands of worthless licensed editions. I am not making it up either. The two have never met, until now. Press on to discover how I ranked the subjective qualities of dozens of notable character, plus a few roster fillers.");
		}//ABOUT
		if (menu_select == 4) {//HOW TO PLAY
			keep_text = true;
			NF_ClearTextLayer(1, 1);
			NF_ClearTextLayer(0, 0);
			NF_WriteText(1, 1, 0, 0, "Unfortunately, this game plays out a lot like how I think Top Trumps works. You select an attribute on your card, and if it's better than your opponent's value, you win their card. Player who runs out of cards first loses. Tie in a round, and both cards get set aside for the next winner. I'm calling it limbo here, because of ghost stuff. If you're looking for deep strategy, don't.");
		}//HOW TO PLAY
	}
...
//strcpy(tlimg, tltest);
    ...
    NF_LoadSpriteGfx((jank[jank_select] + c1img).c_str(), 0, 64, 64);
    NF_LoadSpritePal((jank[jank_select] + c1img).c_str(), 0);
...
NF_CreateSprite(1, 0, 0, 0, 40, 60);
    ...
    NF_LoadSpriteGfx((jank[jank_select] + c2img).c_str(), 4, 64, 64);
    NF_LoadSpritePal((jank[jank_select] + c2img).c_str(), 4);
    // Transfer the required sprites to VRAM
