...
for (int i = 0; i < decksize; i++) {
	deckshuffle[i] = ((rand() + i) % decksize) + 1;
}
int swap1;
int swap2;
int swapcard1;
...
NF_Load16bitsImage("bmp/033", 0, 128, 128);
NF_Load16bitsImage("bmp/033", 1, 128, 128);
...
//Round processing start
	...
	deck1[decksize1] = card1;
	deck1[decksize1 + 1] = card2;
	for (int i = 0; i < decksize2; i++) {
		deck2[i] = deck2[i + 1];
	}
	deck2[decksize2] = card2;
	deck2[decksize2 + 1] = card1;
...
//NF_WriteText(0, 0, 1, 3, "TIE");
	...
	card1 = deck1[0];
	card2 = deck2[0];
	NF_Load16bitsImage("bmp/001" , 0, 128, 128);
	NF_Load16bitsImage("bmp/002" , 1, 128, 128);
	//NF_Load16bitsImage(strcat("bmp/", card_number[card2].c_str()), 1, 128, 128);
	NF_Draw16bitsImage(0, 0, 12, 50, true);
	NF_Draw16bitsImage(1, 1, 12, 50, true);
...
//NF_WriteText(0, 0, 1, 5, str_buffer);
	...
	NF_WriteText(0, 0, 27, 14, "IT ENDED SOMEHOW");//DEBUGGERING ONLY
