...
//?????
	...
	//mSelectMode(MM_MODE_A);
	//mStart( MOD_MUSIC0, MM_PLAY_LOOP );
//?????
...
while (not start_game) {
	...
	if ((keys & KEY_TOUCH) && (touchXY.py > 8) && (touchXY.py < 98)) {//RECALIBRATE
		menu_select = 0;
		if (touchXY.py > 29) {menu_select = 1;}
		if (touchXY.py > 45) {menu_select = 2;}
		if (touchXY.py > 61) {menu_select = 3;}
		if (touchXY.py > 77) {menu_select = 4;}
		if (touchXY.py > 0x87) {keys = 0;}
	} else if (keys & KEY_TOUCH) {keys = 0;}
	...
	if ((keys & KEY_A)||(keys & KEY_B)||(keys & KEY_TOUCH)) {
		...
		if (menu_select == 3) {//ABOUT
			...
			NF_WriteText(0, 0, 0, 0, "Spirited is a 2010 TV series    about a rock star ghost who     falls in love with Australian   dentist. I am not making it up. Top Trumps is a long-dated ghostof a card game with thousands ofworthless licensed editions. I  am not making it up either. The two have never met, until now.  Press on to discover how I      ranked the subjective qualities of dozens of notable character, plus a few roster fillers. Took longer than I thought to make.  I did try for music, but the    compiler was having none of it.");
		}//ABOUT
		if (menu_select == 4) {//HOW TO PLAY
			...
			NF_WriteText(0, 0, 0, 0, "Unfortunately, this game plays  out a lot like how I think Top  Trumps works. You select a valueon your card, and if it's betterthan your opponent's value, you win their card. Player who runs out of cards first loses. Tie ina round, and both cards get set aside for the next winner. I'm  calling it limbo here, because  of ghost stuff. If you're after deep strategy, don't be. The    game was primitive by late      seventies standards, and now    subsists on worthless licenses  and the nostalgia of folks that should know better. They should just print nice cards with      pretty pictures on them instead.");
		}//HOW TO PLAY
	}
	...
	//NF_ClearTextLayer(0, 0);
	//sprintf(str_buffer, "%d", touchXY.px);
	//NF_WriteText(0, 0, 16, 6, str_buffer);
	//sprintf(str_buffer, "%d", touchXY.py);
	//NF_WriteText(0, 0, 18, 10, str_buffer);
	NF_UpdateTextLayers();
	NF_ClearTextLayer(0, 0);
	swiWaitForVBlank();
}
...
player_turn = true;
	...
	if (keys || player_turn == false) {
		...
		if ((keys & KEY_TOUCH) && (touchXY.py > 40) && (touchXY.py < 132) && (touchXY.px > 130)) {//RECALIBRATE
			att_select = 0;
			if (touchXY.py > 60) {att_select = 1;}
			if (touchXY.py > 76) {att_select = 2;}
			if (touchXY.py > 92) {att_select = 3;}
			if (touchXY.py > 108) {att_select = 4;}
		} else if (keys & KEY_TOUCH) {keys = 0;}
		...
		if (keys & KEY_START) {//Pause
			...
			NF_ClearTextLayer(0, 0);
			NF_ClearTextLayer(1, 1);
			NF_WriteText(0, 0, 18, 9, "PAUSE");
			if (menu_select == 0) {
			NF_WriteText(0, 0, 18, 11, "--CONTINUE--");
			NF_WriteText(0, 0, 18, 13, "    QUIT    ");
			} else {
			NF_WriteText(0, 0, 18, 11, "  CONTINUE  ");
			NF_WriteText(0, 0, 18, 13, "----QUIT----");
			}
			NF_UpdateTextLayers();
			if ((keys & KEY_UP)||(keys & KEY_DOWN)) {menu_select = (menu_select + 1) % 2;}
			if ((keys & KEY_A)||(keys & KEY_B)) {
				stay_paused = false;
				if (menu_select == 1) {gameover = true; decksize1 = 0;}
			}
			if (keys & KEY_START) {stay_paused = false;}
			if (keys & KEY_TOUCH) {
				stay_paused = false;
				if ((touchXY.px > 101) && (touchXY.py > 102) && (touchXY.py < 119)) {
					stay_paused = false;
					gameover = true;
					decksize1 = 0;
				}
